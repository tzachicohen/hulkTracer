						...
						__local  SearchFrame *					frame)
{
	...
	uint stackBase = (get_local_id(0) * get_local_size(0) + get_local_id(1)) * c_shortStackDepth; 
	...
	bool hitDetected = trace_ray(&ray,&closestHit,pVetices,pIndices,&sceneBox,pTreeNodes,pNodePolys,pPolygonCache,frame);
	...
	if (hitDetected) {
		...
		for (uint i = 0 ; i < lightCount ;i++)
		{
			...
			if (E_pointLight == pLights[i].color.w) 
			{
				...
				bool hit = trace_ray(&shadowRay,&shadowHit,pVetices,pIndices,&sceneBox,pTreeNodes,pNodePolys,pPolygonCache,frame);
